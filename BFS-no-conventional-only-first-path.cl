...
;;;
;;; BFS no conventional only first path
...
;;; It will return a list with only a path.Only the first path,
;;; that acording with BFS, will be the shortest,
;;; there may be other paths just as short, but no shortest
...
;    (case of the option to search not only the first path, but also all the
...
(defun shortest-path (start end net camino)
  ...
  (defvar opcion)  
  (setq opcion 1)  ;;; Option 1 find only the first path that leads the target
